...
export default function Invalidated() {
  ...
  // ✅ Real-time listener for latest invalidated students
  useEffect(() => {
    ...
    const unsub = onSnapshot(qRef, async (snap) => {
      ...
      // Filter by latest invalidated
      const invalidatedOnly = Array.from(latestMap.values()).filter(
        (d) => d.status !== "validated"
      );

      // Resolve missing IDs if needed
      const list = await Promise.all(
        invalidatedOnly.map(async (d) => {
    ...
    <div className="flex h-screen bg-gray-100 font-poppins">
      ...
      <div className="flex-1 p-10 overflow-y-auto">
            ...
            <tbody>
              {filtered.length > 0 ? (
                ...
                <tr>
                  ...
                  >
                    No currently invalidated students.
                  </td>
                </tr>
              )}
            </tbody>
        ...
        <p className="text-xs text-gray-400 mt-4">
          * Updates live — only the latest invalidated students are shown.
        </p>
      </div>
    </div>
  );
